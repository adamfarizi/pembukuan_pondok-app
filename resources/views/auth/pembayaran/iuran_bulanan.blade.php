...
@section('content')
    ...
    @foreach ($iuran_bulanans as $iuran_bulanan)        
        <div class="modal fade" id="deleteModal{{$iuran_bulanan->id_pembayaran}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle{{$iuran_bulanan->id_pembayaran}}"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    </div>
                    <form action="{{url('/pembayaran/iuran_bulanan/delete/' . $iuran_bulanan->id_pembayaran)}}" id="deleteForm" method="post">
                        @csrf
                        @method('DELETE')
